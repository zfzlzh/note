...
require('script-loader!xlsx-style/dist/xlsx.core.min');//更改样式需改为slsx-style，原来为xlsx
...
function sheet_from_array_of_arrays(data, opts) {
    ...
    for(let i in ws){
        ...
        if (i.indexOf('A') > -1 && ws[i].v && typeof (ws[i].v) == 'string' && ws[i].v.indexOf('&#') > -1 && String(ws[i].v).indexOf('&#10;') < 0 ){
        ...
        if (i.indexOf('A') == -1) {
            ...
            if (String(ws[i].v).indexOf('&#') > -1 && String(ws[i].v).indexOf('&#10;') < 0 ) {
                let arr = ws[i].v.split('&#');
                ws[i].v = arr[0]
...
export function export_json_to_excel(th, jsonData, defaultTitle) {
    ...
    // data.unshift(th);
    console.log(1)
    var ws_name, wb = new Workbook(), ws, colWidth;
    // 可以生成多张表格
    for(let i in data){
        let dataInfo;
        ...
        colWidth = dataInfo.map(row => row.map(val => {
