        ...
        switch (type) {
            case 'Point':
                ...
                if (symbol === 'circle') {
                    ...
                    // 修正聚合度单个非聚合点的事件触发区域
                    var offset = { x: 0, y: 0 };
                    if (options.draw === 'cluster' && data.type != "Feature") offset = options.circleOffset;
...
var drawCluster = {
    draw: function draw(context, dataSet, options) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            context.beginPath();
            if (item.properties && item.properties.cluster) {
                if(options.drawType == "img"){
                    var icon = options.clusterImg;
                    let size = intensity.getSize(item.properties.point_count) < 25 ? 25 : intensity.getSize(item.properties.point_count)
                    context.drawImage(icon, coordinates[0] - size, coordinates[1] - size, size * 2, size * 2);
                }else{
                ...
                if (options.label && options.label.show !== false) {
                    ...
                    var textWidth = context.measureText(text).width;
                    context.fillText(text, coordinates[0] + .1 - textWidth / 2, coordinates[1] + .5 + 4.5);
                }
                ...
                if(options.drawType == "img"){
                    ...
                    let dataI = data[i]
                    var coordinates = dataI.geometry._coordinates || dataI.geometry.coordinates;
    ...
    draw: function draw(context, dataSet, options) {
        ...
        for (var i = 0, len = data.length; i < len; i++) {
            if ( data[i].geometry) {
                ...
                // var icon = data[i].icon || options.icon;
                var icon = new Image()
                icon.src = data[i].icon || options.icon
                let dataI = data[i]
                icon.onload =  ()=> { 
                    var coordinates = dataI.geometry._coordinates || dataI.geometry.coordinates;
                    var x = coordinates[0];
...
Supercluster.prototype.load = function load(points) {
    ...
    for (var z = maxZoom; z >= minZoom; z--) {
        ...
        // console.log('4', JSON.parse(JSON.stringify(this.trees)))
        if (log) {}
    }
...
Supercluster.prototype._cluster = function _cluster(points, zoom) {
    ...
    for (var i = 0; i < points.length; i++) {
        ...
        // console.log('8', JSON.parse(JSON.stringify(this.trees)))
    ...
    createClass(Layer, [{
        ...
        value: function _canvasUpdate(time) {
            ...
            if (self.options.draw === 'cluster') {
                if (self.options.clusterRadius != this.supercluster.options.radius){
                    this.supercluster.options.radius = self.options.clusterRadius
                    this.supercluster.load(this.supercluster.points)
                }
