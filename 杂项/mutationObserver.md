...
let dom = document.getElementById('top')
    let observer = new MutationObserver((mutations) => {
        //需要只执行一次判断里的操作，执行后即断开observer，但是使用forEach可能会导致问题，如下：
        //1。如果一次mutation事件中有多个变化满足条件，forEach会遍历所有变化
        //2.在遍历过程中，即使调用了disconnect()，当前的forEach循环仍会继续执行
        mutations.forEach((mutation) => {
            if(viewer.mprViewer.svg.top){
              console.log(123)//打印了很多次
              crosshair3D.updatePipelines()
              observer.disconnect();
            }
        });
        //可以换成其他循环方式,第一次执行后立即断开observer
        mutations.some((mutation) => {
